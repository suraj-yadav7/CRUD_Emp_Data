...
const Employee = () => {
    ...
    const handleRemove=(id)=>{
        let data=empData.filter((elem)=> elem.id !== id)
        setEmpData(data)
        ...
        }).then((data)=> console.log("the rsponse after delete", data)).catch(error => console.log("Error while deleting data", error))
    }
            ...
            <div className='card-body'>
                <table className='table table-bordered'>
                    <thead className='bg-dark text-white'>
                        ...
                        <tbody>
                                {
                                    empData && empData.map((elem,index+1)=>(
                                        <tr key = {elem.id}>
                                        <td>{index}</td>
                                        <td>{elem.id} </td>
                                        <td>{elem.name} </td>
